...
mkdir SelectedEntries_final/withoutYearFilter Selected_AccessionNos_final/withoutYearFilter
mkdir StatReports/withoutYearFilter
mkdir StatReports/withoutYearFilter/Original StatReports/withoutYearFilter/Filtered
...
for i in ./SourceFiles/*.txt; do
    ...
    # Actual filtration step, here we remove entries without sampling country followed by anything that isn't a primary cultured or uncultured
    cat $i | awk 'FNR>3{print$0}' | awk -F "\t" -v cm="$culture_method" -v sy="$sampling_year" -v sc="$source_country" -v st="$source_tissue" '$sc!=""' | grep -e 'uncultured' -e 'primary' > tmp/$file\_selectedentries.txt
    ...
    cat tmp/tmp_$nooflines.txt > SelectedEntries_final/withoutYearFilter/$file\_woyf_selectedentries_e.txt
    # follwing code makes a list of accession numbers for downloading relevant sequences
    cat SelectedEntries_final/withoutYearFilter/$file\_woyf_selectedentries_e.txt | awk -F "\t" '{print $4}' > Selected_AccessionNos_final/withoutYearFilter/$file\_woyf_accessionlist.txt
    # modification of statistics generating R script for selected sequences
    cat StatGeneration.R | sed -e "s/FileToRead/SelectedEntries_final\/withoutYearFilter\/$file\_woyf_selectedentries_e.txt/g" -e "s/headerfile/Source_e\/$file\_h/g" > StatGeneration_selected_tmp.R
    ...
    mv StatGeneration_source_tmp.pdf StatReports/withoutYearFilter/Original/Stats_$file\_woyf_ori.pdf
    mv StatGeneration_selected_tmp.pdf StatReports/withoutYearFilter/Filtered/Stats_$file\_woyf_sel.pdf
    rm *tmp.R
done
rm -rf Source_e
# Ta da!
